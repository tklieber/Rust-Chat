...
const LISTNER_ADDR: &str = "127.0.0.1:9999"; //Adresse d'écoute 

struct Program {    //Permet de gérer les erreurs par Thread 
    name: String
}
...
#[tokio::main]
async fn main() {
    let listener = match TcpListener::bind(LISTNER_ADDR).await{     //Attend la connexion d'un client
        Err(_) => {
    ...
    let (tx, _rx) = broadcast::channel(10);     //Diffuse le message à tous les receiver (rx) connectés   
    ...
    loop {
        ...
        tokio::spawn(async move {       //Nouveau thread qui implémente la struct program
            ...
            let mut pseudoset = 0;      //Vérifie si l'utilisteur possède un pseudo 
            let mut pseudo: String = String::new();

            let pseudo_msg = chiffrer(String::from("Entrez votre pseudo :"));
            let pseudo_msg = pseudo_msg.as_slice();    //Enregistrement du pseudo envoyé par l'utilisateur
            ...
            loop {
                tokio::select! {
                    resultat_select = reading.read(recved_data) => {
                        ...
                        let new_slice = &recved_data[0..buf_size];  //Slice instancié sur la taille du message reçu
                        let mut recved_string = dechiffrer(&new_slice); //Déchiffrement du message reçu 

                        if recved_string.ends_with('\n') {  //Envoie le message lorsque la touche entrée est saisie
                            recved_string.pop();
                        ...
                        if pseudoset != 1 {
                            ...
                            if recved_string == String::from(":quit"){  //Permet de quitter le chat proprement
                                println!("Utilisateur '{}' avec l'IP {} s'est déconnecté", pseudo, addr);
                                break
                            }

                            println!("{} avec le pseudo '{}' à écrit : {}",addr, pseudo, recved_string); //Affichage du message envoyé 

                            match tx.send((recved_string, addr, pseudo.clone())) { //Envoie du message aux Receiver (rx)
                                Err(e) => program.print_error(e.to_string()),
                                _ => continue,
                            } //-> rx à besoin d'un &[u8]
                        }
                    ...
                    resultat_select = rx.recv() => {
                        ...
                        if addr != other_addresses { //Evite de renvoyer le message à l'expediteur 
